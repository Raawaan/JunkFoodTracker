...
interface UserProductDao{
    ...
    @Query("select counter from UserProduct WHERE userId=:id AND productBarcode=:barcode AND date=:date")
    fun sameUserSameProduct(id:Int,barcode:Long,date: Date):Int
    ...
    @Query("SELECT brandName,counter FROM UserProduct,product where UserProduct.userId=:uId AND userProduct.productBarcode=product.barcode AND  UserProduct.date=:date")
